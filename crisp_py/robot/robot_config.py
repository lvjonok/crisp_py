...
@dataclass
class FrankaEmikaPandaConfig(RobotConfig):
...
def make_robot_config(robot_type: str, **kwargs) -> RobotConfig:  # noqa: ANN003
    ...
    elif robot_type == "franka_emika_panda":
        return FrankaEmikaPandaConfig(**kwargs)
    else:
        raise ValueError(
            f"Unsupported robot type: {robot_type}. Supported types: franka, panda, kinova, iiwa, so101, franka_emika_panda"
        )
